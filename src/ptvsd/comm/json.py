...
__all__ = ['JsonIOStream', 'JsonMessageChannel', 'MessageHandlers', 'RequestFailure']
...
class JsonIOStream(object):
    ...
    def read_json(self):
        ...
        Returns JSON value as parsed by json.loads(), or raises ConnectionError
...
class Request(object):
    ...
    Incoming requests are represented by instances of IncomingRequest, which
    provides additional functionality to send responses.
...
class IncomingRequest(Request):
    """Represents an incoming request, to which it is possible to respond.
    """

    def __init__(self, *args):
        super(IncomingRequest, self).__init__(*args)
        self.responded = False

    def respond(self, response_body=None):
        assert not self.responded
        self.responded = True
        if isinstance(response_body, Exception):
            self.channel._send_response(self.seq, False, self.command, str(response_body), None)
        else:
            self.channel._send_response(self.seq, True, self.command, None, response_body)

    def fail(self, message):
        self.respond(RequestFailure(message))
...
class Response(object):
    ...
    # A token object that a request handler can return in lieu of the response body
    # to indicate that it intends to handle the request asynchronously by calling
    # IncomingRequest.respond directly.
    ASYNC = object()
    ...
    def __init__(self, channel, seq, request, body):
        ...
        disconnected before sending a response), it is ConnectionError.
        """
...
class JsonMessageChannel(object):
    ...
    def __init__(self, stream, handlers=None, name='vsc_messaging'):
        ...
        self._worker = ptvsd.util.internal_thread(name, self._process_incoming_messages)
    ...
    def on_request(self, seq, command, arguments):
        ...
        request = IncomingRequest(self, seq, command, arguments)
        try:
            response_body = handler(request)
        except RequestFailure as ex:
            response_body = ex
        if response_body is not Response.ASYNC:
    ...
    def on_disconnect(self):
        ...
        with self._lock:
            for request in self._requests.values():
                request._handle_response(None, request.command, ptvsd.comm.ConnectionError('No response'))
